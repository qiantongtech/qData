<template>
  <div class="app-container" ref="app-container">
    <el-container style="90%">
      ...
      <el-main>
        ...
        <div class="pagecont-bottom">
          ...
          <el-table stripe height="60vh" v-loading="loading" :data="DppQualityTaskEvaluateList">
            <el-table-column v-if="getColumnVisibility(8)" label="编号" align="center" prop="id" width="80" />
            <el-table-column v-if="getColumnVisibility(1)" label="任务名称" align="center" prop="taskName"
            ...
            <el-table-column v-if="getColumnVisibility(2)" label="稽查对象数" align="center" prop="taskObjNum" width="80"
              show-overflow-tooltip>
              <template #default="scope">
                {{ scope.row.taskObjNum || '-' }}
              </template>
            </el-table-column>
            <el-table-column v-if="getColumnVisibility(3)" label="稽查规则数" align="center" prop="taskEvaluateNum"
              width="90" show-overflow-tooltip>
              <template #default="scope">
                {{ scope.row.taskEvaluateNum || '-' }}
              </template>
            </el-table-column>
            <el-table-column v-if="getColumnVisibility(5)" label="任务状态" align="center" prop="status"
              show-overflow-tooltip>
              <template #default="scope">
            ...
            <el-table-column v-if="getColumnVisibility(6)" label="执行策略" align="center" prop="strategy">
              <template #default="scope">
                <dict-tag :options="dpp_etl_task_execution_type" :value="scope.row.strategy" />
              </template>
            </el-table-column>

            <el-table-column v-if="getColumnVisibility(7)" label="所属分类" align="center" prop="catName"
              show-overflow-tooltip width="200">
              <template #default="scope">
                {{ scope.row.catName || '-' }}
              </template>
            </el-table-column>
            <el-table-column v-if="getColumnVisibility(5)" label="调度周期" align="center" prop="cycle"
              show-overflow-tooltip width="240">
              <template #default="scope">
                {{ cronToZh(scope.row.cycle) || "-" }}
              </template>
            </el-table-column>
            <el-table-column v-if="getColumnVisibility(4)" label="上次执行时间" align="center" prop="lastExecuteTime"
            ...
            <el-table-column label="操作" align="center" class-name="small-padding fixed-width" fixed="right" width="220">
              <template #default="scope">
                ...
                <el-button link type="primary" icon="Edit" @click="routeTo('/da/dataQuality/dataQualityTasks/components/qualityTaskEdit', {
                  ...scope.row,
                })" v-hasPermi="['dpp:qa:dppqualitytaskevaluate:edit']" :disabled="scope.row.status != 1">
                  配置</el-button>
                <el-button link type="primary" icon="view" @click="
                  routeTo('/da/dataQuality/dataQualityTasks/components/qualityTaskEdit', {
                    ...scope.row,
                    info: true,
                  })
                  " v-hasPermi="['dpp:etl:ddv:info']">详情</el-button>
    ...
    <qualityConfigDialog :visible="taskConfigDialogVisible" title="新增任务"
      @update:visible="taskConfigDialogVisible = $event" @save="handleSave" @confirm="handleConfirm" :data="taskForm"
      :userList="userList" :deptOptions="deptOptions" :info="route.query.info" />
  </div>
</template>
...
// 列显隐信息
const columns = ref([
  { key: 8, label: "编号", visible: true },
  { key: 1, label: "任务名称", visible: true },
  { key: 2, label: "稽查对象数", visible: true },
  { key: 3, label: "稽查规则数", visible: true },
  { key: 4, label: "最后执行时间", visible: true },
  { key: 5, label: "调度周期", visible: true },
  { key: 6, label: "执行策略", visible: true },
  { key: 7, label: "所属分类", visible: true },
]);
