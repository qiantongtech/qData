                    ...
                    if (isUpdateSupport) {
                        if (daDiscoveryTaskLogId != null) {
                            ...
                            if (existingDaDiscoveryTaskLog != null) {
                                ...
                                successNum++;
                                successMessages.add("数据更新成功，ID为 " + daDiscoveryTaskLogId + " 的数据发现任务日志记录。");
                            } else {
                                failureNum++;
                                failureMessages.add("数据更新失败，ID为 " + daDiscoveryTaskLogId + " 的数据发现任务日志记录不存在。");
                            }
                        } else {
                            failureNum++;
                        ...
                        if (existingDaDiscoveryTaskLog == null) {
                            ...
                            successNum++;
                            successMessages.add("数据插入成功，ID为 " + daDiscoveryTaskLogId + " 的数据发现任务日志记录。");
                        } else {
                            failureNum++;
                            failureMessages.add("数据插入失败，ID为 " + daDiscoveryTaskLogId + " 的数据发现任务日志记录已存在。");
                        }
                    }
                } catch (Exception e) {
                    failureNum++;
