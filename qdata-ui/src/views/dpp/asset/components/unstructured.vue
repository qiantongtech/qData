<template>
  <el-row :gutter="20">
    <el-col :span="12">
      <el-form-item label="数据连接名称" prop="datasourceId" :rules="[{ required: true, message: '请选择数据连接名称', trigger: 'change' }]">
        <el-select
  ...
  <el-row :gutter="20" v-if="localForm.filePath">
    <el-col :span="24">
      <el-descriptions title="" :column="2" border>
        <el-descriptions-item v-for="(item, index) in fileDesc" :key="index" label-class-name="base-label" class-name="base-content">
          <template #label>
  ...
  <el-dialog class="file-dialog" title="选择文件" width="900px" v-model="visibleDialog" draggable destroy-on-close :append-to="$refs['app-container']">
        ...
        <div class="back">
          ...
          <div class="catalogue">
            ...
            <el-text type="primary" @click="handleCatalogue(item)" v-for="(item, index) in catalogues" :key="item">
              <span class="catalogue-text">{{ item }}</span> <span class="catalogue-split" v-if="index != catalogues.length - 1"> / </span>
            </el-text>
          </div>
        </div>
      </div>
      <!-- :tree-props="{ children: 'children', hasChildren: 'hasChildren' }" -->
      <el-table height="380px" v-loading="fileListLoading" :data="currentPageData" row-key="id" @selection-change="handleSelectionChange" @row-click="handleRowClick">
        <el-table-column type="selection" width="55" :selectable="selectable" />
        <el-table-column label="文件名" prop="name" show-overflow-tooltip>
          <template #default="scope">
            <div class="fileName">
              <img v-if="scope.row.directory" src="@/assets/da/asset/icon (3).png" alt="" />
              <el-icon v-else>
                <Tickets />
              </el-icon>
        ...
        <el-table-column label="文件大小" prop="size" show-overflow-tooltip align="center">
          <template #default="scope">
            <span>{{ scope.row.directory ? "-" : (scope.row.size / 1024).toFixed(2) + "KB" || "-" }}</span>
          </template>
        </el-table-column>
        <el-table-column label="更新时间" prop="lastModified" show-overflow-tooltip align="center">
          <template #default="scope">
            {{ scope.row.lastModified || "-" }}
          </template>
        </el-table-column>
      </el-table>
      <pagination v-show="fileList.length > 0" :total="fileList.length" v-model:page="queryParams.pageNum" v-model:limit="queryParams.pageSize" />
    </div>
    <template #footer>
...
import { getFileList } from "@/api/da/asset/daAsset";
