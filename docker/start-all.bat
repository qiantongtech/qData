@echo off
REM =============================================================================
REM qData Full-Mode Docker Launcher
REM
REM Purpose:
REM   Validates the local Docker environment and deployment package, initializes
REM   the configured database, and starts the complete qData service stack.
REM
REM Usage:
REM   Run start-all.bat from Command Prompt or double-click it in File Explorer.
REM
REM Requirements:
...
REM Define the full-mode profile, project identity, and default configuration files.
set "QDATA_MODE=all"
...
REM Pull all images required by the full deployment profile.
call :pull_images
if errorlevel 1 goto :failed
...
REM Start the complete qData application and scheduler stack.
...
REM Report container status and print service access credentials.
...
REM Prepare the host and deployment configuration for a full-mode startup.
:prepare
...
REM Verify that Docker Desktop has enough CPU and memory for full mode.
...
REM Validate the application and scheduler artifacts required by full mode.
...
if not exist "dolphinscheduler\resource\default\resources\spark-jar\qdata-executor-etl*.jar" (
...
REM Check Docker resource allocations against the full-mode recommendations.
:check_resources
...
REM Compare the detected allocations with the full-mode minimums.
...
REM Pull full-mode images with bounded retries for transient registry failures.
:pull_images
