...
| シナリオ | 説明 | 典型的な顧客タイプ |
| --- | --- | --- |
| **ETLデータ連携** | データアクセス、クレンジング、変換、出力のワークフローを可視的に設定可能。タスクはシナリオに応じてDataXまたはSpark実行エンジンを選択でき、軽量同期、関連データ取得、データインポート、大規模オフライン処理をサポート。 | データ開発チーム、ソフトウェア企業 |
| **データ開発スケジューリング** | SQLデータ処理タスク開発をサポート。新規タスク作成時にQuartzまたはDolphinSchedulerを選択でき、軽量な定期スケジューリングと可視的タスクオーケストレーションを両立。 | データ開発チーム、プラットフォーム運用チーム |
| **データガバナンス構築** | データ標準、データモデル、メタデータ、データ品質、データ資産を統合管理し、基礎ガバナンス体系を構築。 | 政府機関、グループ企業、研究機関・大学 |
| **メタデータ収集管理** | メタデータ収集タスクは実際のデプロイ形態に応じてQuartzまたはDolphinSchedulerによるスケジューリングを選択でき、収集実行戦略を柔軟に設定可能。 | データガバナンスチーム、データ管理部門 |
...
| **可視化ETL** | データアクセス、クレンジング、変換、出力のワークフローを可視的に設定でき、DataXとSparkの実行エンジンを選択可能。異なる規模のデータ連携タスクの開発敷居を下げる。 |
| **柔軟な実行エンジン** | DataXは単一ノードの軽量データ同期、関連データ取得、データインポートに適し、Sparkは大規模オフライン処理と複雑な計算タスクに適している。 |
| **選択可能なスケジューリング方式** | データ開発タスクとメタデータ収集タスクはQuartzまたはDolphinSchedulerを選択でき、軽量なローカルスケジューリングと集中型タスクオーケストレーション、依存管理、スケジューリング管理の両方をサポート。 |
...
| モジュール | 説明 |
| --- | --- |
| **データ連携（ETL）** | データアクセス、クレンジング、変換、出力のワークフローを可視的に設定可能。新規タスク作成時にDataXまたはSpark実行エンジンを選択でき、軽量データ同期、関連データ取得、データインポート、大規模オフライン処理、複雑な計算シナリオに適用。 |
...
<table>
  ...
  <tr>
    <td rowspan="6">サードパーティ依存</td><td>DolphinScheduler</td><td>可視的タスクオーケストレーション、依存管理、スケジューリング機能を提供し、データ開発タスクとメタデータ収集タスクのスケジューリングに利用可能</td>
  </tr>
  <tr>
