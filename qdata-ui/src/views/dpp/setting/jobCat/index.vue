    ...
    <div class="app-container" ref="app-container">
        <div class="pagecont-top" v-show="showSearch">
            <el-form class="btn-style" :model="queryParams" ref="queryRef" :inline="true" label-width="75px"
                ...
                <el-form-item label="作业类目名称" prop="name" label-width="130">
                    <el-input class="el-form-input-width" v-model="queryParams.name" placeholder="请输入作业类目名称" clearable
                        @keyup.enter="handleQuery" />
                </el-form-item>
                <el-form-item label="上级类目" prop="code">
                    <el-tree-select filterable class="el-form-input-width" v-model="queryParams.code"
                        :data="attJobCatOptions" :props="{ value: 'code', label: 'name', children: 'children' }"
        ...
        <div class="pagecont-bottom">
            <div class="justify-between mb15">
                <el-row :gutter="15" class="btn-style">
                    <el-col :span="1.5">
                        <el-button type="primary" plain @click="handleAdd" v-hasPermi="['dpp:developInstance:add']"
            ...
            <el-table height="60vh" v-if="refreshTable" v-loading="loading" :data="AttJobCatList" row-key="id"
                :default-expand-all="isExpandAll" :tree-props="{ children: 'children', hasChildren: 'hasChildren' }">
                <el-table-column label="作业类目名称" align="left" prop="name" width="200"
                ...
                <el-table-column label="操作" align="center" class-name="small-padding fixed-width" fixed="right"
                    ...
                    <template #default="scope">
                        <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)"
                            v-hasPermi="['dpp:developInstance:edit']">修改</el-button>
                        <el-button link type="primary" icon="Plus" @click="handleAdd(scope.row)"
                            v-hasPermi="['dpp:developInstance:add']">新增</el-button>
                        <el-button link type="danger" icon="Delete" @click="handleDelete(scope.row)"
                            v-hasPermi="['dpp:developInstance:remove']">删除</el-button>
                    </template>
                </el-table-column>
            </el-table>
        ...
        <el-dialog :title="title" v-model="open" width="800px" :append-to="$refs['app-container']" draggable>
            ...
            <el-form ref="AttJobCatRef" :model="form" :rules="rules" label-width="80px" @submit.prevent>
                <el-row :gutter="20">
                    <el-col :span="12">
                        <el-form-item label="类目名称" prop="name">
                            <el-input v-model="form.name" placeholder="请输入作业类目名称" />
                        </el-form-item>
                    </el-col>
                    <el-col :span="12">
                        <el-form-item label="上级类目" prop="parentId">
                            <el-tree-select filterable :disabled="form.id" v-model="form.parentId"
                                :data="attJobCatOptions" :props="{ value: 'id', label: 'name', children: 'children' }"
                                value-key="id" placeholder="请选择上级" check-strictly />
                        </el-form-item>
                    </el-col>
                </el-row>
        ...
        <el-dialog :title="title" v-model="openDetail" width="800px" :append-to="$refs['app-container']" draggable>
            ...
            <el-form ref="AttJobCatRef" :model="form" label-width="80px">
                <el-row :gutter="20">
                    <el-col :span="12">
                ...
                <template #tip>
                    <div class="el-upload__tip text-center">
                        <div class="el-upload__tip">
                            <el-checkbox v-model="upload.updateSupport" />是否更新已经存在的作业类目管理数据
                        </div>
...
import {
    ...
    addAttJobCat,
    updateAttJobCat
} from '@/api/att/cat/jobCat/jobCat';
import { getToken } from '@/utils/auth.js';
import useUserStore from '@/store/system/user';
const userStore = useUserStore();
const { proxy } = getCurrentInstance();

const AttJobCatList = ref([]);
...
const upload = reactive({
    ...
    // 上传的地址
    url: import.meta.env.VITE_APP_BASE_API + '/att/attJobCat/importData'
});
...
const data = reactive({
    ...
    rules: {
        name: [{ required: true, message: '作业类目名称不能为空', trigger: 'blur' }],
...
function getList() {
    ...
    listAttJobCat(queryParams.value).then((response) => {
        AttJobCatList.value = proxy.handleTree(response.data, 'id', 'parentId');
        total.value = response.data.total;
        loading.value = false;

        attJobCatOptions.value = [];
        const data = { id: 0, name: '顶级节点', children: [] };
        data.children = proxy.handleTree(response.data, 'id', 'parentId');
        attJobCatOptions.value.push(data);
    });
}
...
function handleUpdate(row) {
    ...
    getAttJobCat(_id).then((response) => {
        form.value = response.data;
        open.value = true;
        title.value = '修改作业类目管理';
    });
}
...
function handleDetail(row) {
    ...
    getAttJobCat(_id).then((response) => {
        ...
        title.value = '作业类目管理详情';
    });
}

/** 提交按钮 */
function submitForm() {
    proxy.$refs['AttJobCatRef'].validate((valid) => {
        if (valid) {
            if (form.value.id != null) {
                updateAttJobCat(form.value)
                    .then((response) => {
...
function handleStatusChange(row) {
    ...
    proxy.$modal
        .confirm('确认要"' + text + '","' + row.name + '"作业类目吗？')
        .then(function () {
            updateAttJobCat({ id: row.id, validFlag: row.validFlag }).then((response) => {
...
function handleDelete(row) {
    ...
    proxy.$modal
        .confirm('是否确认删除作业类目管理编号为"' + row.name + '"的数据项？')
        .then(function () {
            return delAttJobCat(_ids);
        })
        .then(() => {
            getList();
...
function handleExport() {
    proxy.download(
        'att/AttJobCat/export',
        {
            ...queryParams.value
        },
        `AttJobCat_${new Date().getTime()}.xlsx`
    );
}
...
function importTemplate() {
    proxy.download(
        ...
        {},
        `AttJobCat_template_${new Date().getTime()}.xlsx`
    );
}
