  ...
  <el-dialog v-model="visibleDialog" :draggable="true" class="medium-dialog" :title="currentNode?.data?.name"
    ...
    <el-form ref="dpModelRefs" :model="form" label-width="110px" @submit.prevent v-loading="loading" :disabled="info">
      ...
      <el-divider content-position="center">
        <span class="blue-text">需要操作的字段</span>
      </el-divider>
      ...
      <el-table stripe height="310px" :data="tableFields" v-loading="loadingList" ref="dragTable" row-key="columnName">
        ...
        <el-table-column label="输出字段" align="left" prop="outputField" :show-overflow-tooltip="{ effect: 'light' }"
          width="150">
          <template #default="scope">
            <el-input v-model.trim="scope.row.outputField" placeholder="请输入" style="width: 100%" />
          </template>
        </el-table-column>
        <el-table-column label="去空格方式" align="left" prop="trimType" width="150">
          <template #default="scope">
            <el-select v-model="scope.row.trimType" placeholder="请选择" style="width: 100%">
              <el-option label="不去空格" value="none" />
              <el-option label="去除左侧空格" value="left" />
              <el-option label="去除右侧空格" value="right" />
              <el-option label="去除两侧空格" value="both" />
            </el-select>
          </template>
        </el-table-column>
        <el-table-column label="大小写转换" align="left" prop="lowerUpper" width="150">
          <template #default="scope">
            <el-select v-model="scope.row.lowerUpper" placeholder="请选择" style="width: 100%">
        ...
        <el-table-column label="填充方向" align="left" prop="padding">
          <template #default="scope">
            <el-select v-model="scope.row.padding" placeholder="请选择" style="width: 100%">
              <el-option label="不填充" value="none" />
              <el-option label="填充左侧" value="left" />
              <el-option label="填充右侧" value="right" />
              <el-option label="填充两侧" value="both" />
            </el-select>
          </template>
        </el-table-column>
        <el-table-column label="填充字符" align="left" prop="padChar" width="150">
          <template #header>
            <div class="justify-center">
              <span>填充字符</span>
              <el-tooltip effect="dark" content="如果使用填充功能，指定填充所使用的字符" placement="top">
                <el-icon>
                  <InfoFilled />
                </el-icon>
              </el-tooltip>
          ...
          <template #default="scope">
            <el-input v-model="scope.row.padChar" placeholder="请输入" style="width: 100%" />
          </template>
        </el-table-column>
