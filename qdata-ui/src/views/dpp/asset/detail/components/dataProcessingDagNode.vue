<template>
  <div class="data-processing-dag-node">
    <div class="main-area" :class="{ act: nodeData.active }" @mouseenter="onMainMouseEnter" @mouseleave="onMainMouseLeave">
      <div class="main-info">
        <!-- {/* 节点类型icon */} -->
        ...
        <el-popover :disabled="!currentNode.name && !currentNode.status" width="auto" title="" content="" placement="top">
          <template #default>
            <template v-if="nodeData.name == currentNode.name && currentNode.status === CellStatus.ERROR">
              <div class="pop-class" @mouseenter="currentNode = nodeData" @mouseleave="currentNode = {}">
                <div class="li">数据表名称：{{ nodeData.name }}</div>
                <div class="li">数据库类型：{{ nodeData.descName }}</div>
                <div class="li">数据连接名称：名称</div>
                <div class="li">表状态：{{ nodeData.status }}</div>
              </div>
            </template>
            <template v-if="nodeData.name == currentNode.name && currentNode.status === CellStatus.SUCCESS">
              <div class="pop-class" @mouseenter="currentNode = nodeData" @mouseleave="currentNode = {}">
                <div class="li">任务节点名称：{{ nodeData.name }}</div>
                <div class="li">任务类型：{{ nodeData.descName }}</div>
                <div class="li">上一次执行时间：16:03</div>
                <div class="li">上一次执行状态：{{ nodeData.status }}</div>
                <div class="li">任务状态：{{ nodeData.status }}</div>
              </div>
            </template>
          </template>
          <template #reference>
            <div class="main-text" @mouseenter="currentNode = nodeData" @mouseleave="currentNode = {}">
              ...
              <div class="ellipsis-row node-desc">{{ nodeData.descName }}</div>
            </div>
          </template>
        </el-popover>
      </div>
    ...
    <template v-if="nodeData.leaf">
      <div class="plus-dag">
        <el-icon @click="handleCollapse(nodeData)" v-show="nodeData.collapsed"><Remove /></el-icon>
        <el-icon @click="handleCollapse(nodeData)" v-show="!nodeData.collapsed"><CirclePlus /></el-icon>
      </div>
    </template>
