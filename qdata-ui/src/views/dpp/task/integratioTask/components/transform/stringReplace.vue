  ...
  <el-dialog v-model="visibleDialog" :draggable="true" class="medium-dialog" :title="currentNode?.data?.name"
    ...
    <el-form ref="dpModelRefs" :model="form" label-width="110px" @submit.prevent v-loading="loading" :disabled="info">
      ...
      <el-divider content-position="center">
        <span class="blue-text">需要替换的字段</span>
      </el-divider>
      ...
      <el-table stripe height="310px" :data="tableFields" v-loading="loadingList" ref="dragTable" row-key="columnName">
        ...
        <el-table-column label="输入字段" align="left" prop="columnName" width="150">
          <template #default="scope">
            <el-select v-model="scope.row.columnName" placeholder="请选择" style="flex: 1">
        ...
        <el-table-column label="输出字段" align="left" prop="outputField" :show-overflow-tooltip="{ effect: 'light' }"
          width="150">
          <template #default="scope">
            <el-input v-model="scope.row.outputField" placeholder="请输入" style="width: 100%" />
          </template>
        </el-table-column>
        <el-table-column label="使用正则表达式" align="left" prop="useRegex" width="150">
          <template #default="scope">
            <el-select v-model="scope.row.useRegex" placeholder="请选择" style="width: 100%"
        ...
        <el-table-column label="搜索" align="left" prop="search" width="150">
          <template #default="scope">
            <el-input v-model="scope.row.search" placeholder="请输入" style="width: 100%" />
          </template>
        </el-table-column>
        <el-table-column label="替换成的内容" align="left" prop="replaceWith" width="150">
          <template #default="scope">
            <el-input v-model.trim="scope.row.replaceWith" placeholder="请输入" style="width: 100%"
              @input="handleReplaceWith(scope.row)" />
          </template>
        </el-table-column>
        <el-table-column label="设置为空串" align="left" prop="setEmpty" width="150">
          <template #header>
            <div class="justify-center">
              <span>设置为空串</span>
              <el-tooltip effect="dark" content="是否将匹配结果替换为空字符串" placement="top">
                <el-icon>
                  <InfoFilled />
                </el-icon>
              </el-tooltip>
          ...
          <template #default="scope">
            <el-select v-model="scope.row.setEmpty" placeholder="请选择" style="width: 100%"
              @change="handleSetEmpty(scope.row)">
              <el-option label="是" :value="true" />
              <el-option label="否" :value="false" />
            </el-select>
          </template>
        </el-table-column>
        <el-table-column label="使用字段值替换" align="left" prop="useFieldValue " width="150">
          <template #header>
            <div class="justify-center">
              <span>使用字段值替换</span>
              <el-tooltip effect="dark" content="是否使用另一个字段的值作为替换内容" placement="top">
                <el-icon>
                  <InfoFilled />
                </el-icon>
              </el-tooltip>
          ...
          <template #default="scope">
            <el-select v-model="scope.row.useFieldValue" placeholder="请选择" style="flex: 1"
              @change="handleUseFieldValue(scope.row)" clearable>
              <el-option v-for="item in inputFields" :key="item.value" :label="item.label" :value="item.columnName"
                :disabled="isRowDisabled(item.columnName, scope.row)" />
            </el-select>
          </template>
        </el-table-column>
        <el-table-column label="整个单词匹配" align="left" prop="matchWholeWord" width="150">
          <template #header>
            <div class="justify-center">
              <span>整个单词匹配</span>
              <el-tooltip effect="dark" content="是否整词匹配，仅完全匹配整词时才替换" placement="top">
                <el-icon>
                  <InfoFilled />
                </el-icon>
              </el-tooltip>
          ...
          <template #default="scope">
            <el-select v-model="scope.row.matchWholeWord" placeholder="请选择" style="width: 100%"
              :disabled="scope.row.useRegex">
              <el-option label="是" :value="true" />
              <el-option label="否" :value="false" />
            </el-select>
          </template>
        </el-table-column>
        <el-table-column label="大小写敏感" align="left" prop="caseSensitive" width="150">
          <template #header>
            <div class="justify-center">
              <span>大小写敏感</span>
              <el-tooltip effect="dark" content="是否区分大小写" placement="top">
                <el-icon>
                  <InfoFilled />
                </el-icon>
              </el-tooltip>
          ...
          <template #default="scope">
            <el-select v-model="scope.row.caseSensitive" placeholder="请选择" style="width: 100%"
              :disabled="scope.row.useRegex">
              <el-option label="是" :value="true" />
              <el-option label="否" :value="false" />
            </el-select>
...
const handleRegexChange = (row) => {
  if (row.useRegex) {
    row.matchWholeWord = false;
    row.caseSensitive = false;
  }
};
const handleReplaceWith = (row) => {
  if (row.replaceWith.length > 0) {
    row.setEmpty = false;
    row.useFieldValue = "";
  }
}
const handleSetEmpty = (row) => {
  if (row.setEmpty) {
    row.replaceWith = "";
    row.useFieldValue = "";
  }
}
const handleUseFieldValue = (row) => {
  if (row.useFieldValue.length > 0) {
    row.replaceWith = "";
    row.setEmpty = false;
  }
}
...
const isOptionDisabled = (optionValue, currentRow) => {
  return tableFields.value.some((row) => row !== currentRow && row.columnName === optionValue);
};

const isRowDisabled = (optionValue, currentRow) => {
  return currentRow.columnName === optionValue;
};
...
const saveData = async () => {
  try {
    ...
    taskParams.tableFields = tableFields.value.map((field) => ({ ...field, inputField: field.columnName }));
    ...
    let outputFields = inputFields.value.map((input) => {
      ...
      return matched ? { ...input, ...matched, inputField: input.columnName } : { ...input, inputField: input.columnName };
    });
    let addOutputField = outputFields
      .filter((item) => item.outputField)
      .map((item) => {
        return { ...item, columnName: item.outputField };
      });
    taskParams.outputFields = [...outputFields, ...addOutputField];
