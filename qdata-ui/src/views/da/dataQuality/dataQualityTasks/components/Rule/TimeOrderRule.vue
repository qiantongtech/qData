...
const form = reactive({
    ...
    conditions: props.form.conditions?.conditions?.length
        ? JSON.parse(JSON.stringify(props.form.conditions.conditions))
...
function validateCalculationGroups() {
    if (form.conditions.length === 0) {
        ElMessage.warning("请至少添加一组计算条件");
        return false;
    }
    for (let i = 0; i < form.conditions.length; i++) {
        const group = form.conditions[i];
        if (!group.leftField) {
            ElMessage.warning(`请填写第 ${i + 1} 个计算组的左字段`);
            return false;
        }
        if (!group.operator || !["<", "≤"].includes(group.operator)) {
            ElMessage.warning(`第 ${i + 1} 个计算组的符号无效`);
            return false;
        }
        if (!group.rightField) {
            ElMessage.warning(`请填写第 ${i + 1} 个计算组的右字段`);
            return false;
        }
    }
    return true;
