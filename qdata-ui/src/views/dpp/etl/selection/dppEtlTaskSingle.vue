<template>
  <el-dialog title="数据集成任务-单选" v-model="visible" width="1200px" :append-to="$refs['app-container']" draggable
...
<script setup name="DppEtlTaskSingle">
...
function getList() {
  loading.value = true;
  listDppEtlTask(proxy.addDateRange(queryParams.value, daterangeCreateTime.value)).then(
    async (response) => {
