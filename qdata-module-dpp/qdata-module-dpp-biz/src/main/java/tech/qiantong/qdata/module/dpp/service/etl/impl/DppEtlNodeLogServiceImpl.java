                ...
                if (isUpdateSupport) {
                    if (dppEtlNodeLogId != null) {
                        ...
                        if (existingDppEtlNodeLog != null) {
                            ...
                            successMessages.add(MessageUtils.messageWithFallback("dpp.import.update.success",
                                    "Data update successful, ID {0} {1} record.", dppEtlNodeLogId, MessageUtils.messageWithFallback("dpp.entity.etl.node.log", "Data integration node log")));
                        } else {
                            failureNum++;
                            failureMessages.add(MessageUtils.messageWithFallback("dpp.import.update.fail",
                                    "Data update failed, ID {0} {1} record does not exist.", dppEtlNodeLogId, MessageUtils.messageWithFallback("dpp.entity.etl.node.log", "Data integration node log")));
                        }
                    } else {
                        failureNum++;
                    ...
                    if (existingDppEtlNodeLog == null) {
                        ...
                        successMessages.add(MessageUtils.messageWithFallback("dpp.import.insert.success",
                                "Data insert successful, ID {0} {1} record.", dppEtlNodeLogId, MessageUtils.messageWithFallback("dpp.entity.etl.node.log", "Data integration node log")));
                    } else {
                        failureNum++;
                        failureMessages.add(MessageUtils.messageWithFallback("dpp.import.insert.fail",
                                "Data insert failed, ID {0} {1} record already exists.", dppEtlNodeLogId, MessageUtils.messageWithFallback("dpp.entity.etl.node.log", "Data integration node log")));
                    }
                }
            } catch (Exception e) {
                failureNum++;
