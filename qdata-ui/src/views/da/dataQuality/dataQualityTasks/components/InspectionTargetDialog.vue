<template>
    <el-dialog v-model="dialogVisible" draggable class="dialog" :title="dialogTitle" destroy-on-close width="60%"
            ...
            <el-row :gutter="20">
                <el-col :span="12">
                    <el-form-item label="数据源类型">
                        <el-input v-model="form.datasourceType" disabled placeholder="数据源类型" />
                    </el-form-item>
                </el-col>
                <el-col :span="12">
                    <el-form-item label="数据源实例">
                        <el-input v-model="form.dbname" disabled placeholder="数据源实例" />
                    </el-form-item>
                </el-col>
            </el-row>
            <el-row :gutter="20">
...
import { getTablesByDataSourceId, getColumnByAssetId } from '@/api/dpp/etl/dppEtlTask'
...
const onTableChange = async (val) => {
    ...
    if (selectedTable) {
        form.columnComment = selectedTable.tableComment; // 保存中文名
    } else {
        form.tableComment = '';
    }
};
