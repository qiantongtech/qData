    ...
    <div class="app-container" ref="app-container">
        ...
        <div class="pagecont-bottom">
            ...
            <el-table height="60vh" v-if="refreshTable" v-loading="loading" :data="attDataElemCatList" row-key="id"
                ...
                <!--            </template>-->
                <!--          </el-table-column>-->
                <el-table-column :label="td('att.attDocCat.table.name')" align="left" prop="name" width="200"
                ...
                <el-table-column :label="td('common.texts.operation')" align="center" class-name="small-padding fixed-width" fixed="right"
                    ...
                    <template #default="scope">
                        <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)"
                            v-hasPermi="['att:cat:doccat:edit']">{{ td('common.button.update') }}</el-button>
                    ...
                    <el-col :span="12">
                        <el-form-item :label="td('att.common.parentCat')" prop="parentId" :label-position="labelPosition">
                            <el-tree-select filterable :disabled="form.id" v-model="form.parentId"
                                ...
                                :placeholder="td('att.common.parentCatPlaceholder')" check-strictly />
                        </el-form-item>
                    </el-col>
                </el-row>
                <el-row :gutter="20">
...
const data = reactive({
    ...
    rules: {
        name: [{ required: true, message: td('att.attDocCat.validations.nameRequired'), trigger: 'blur' }],
...
function handleStatusChange(row) {
    ...
    proxy.$modal
        .confirm(td('att.common.confirmStatusChangeGeneric', '', { status: text, type: td('att.common.dataDoc'), name: row.name }))
        .then(function () {
...
function handleDelete(row) {
    proxy.$modal
        .confirm(td('att.attDocCat.messages.confirmDelete', '', { name: row.name }))
        .then(function () {
