  ...
  <el-dialog v-model="visible" :title="title" width="800px" :append-to="$refs['app-container']" draggable
    ...
    <el-form ref="form" :model="form" :rules="rules" label-width="125px" v-loading="loading">
      ...
      <el-form-item label="表单编码" prop="desformCode">
        <el-input v-model="form.desformCode" style="width: 100%" placeholder="请输入表单编码" disabled />
      </el-form-item>
      ...
      <template v-if="form.saveTableFlag == 1">
        <el-form-item label="目标数据库源" prop="datasourceId">
          <el-select v-model="form.datasourceId" filterable style="width: 100%" @change="rDbChange" disabled>
        ...
        <el-form-item label="目标数据库" prop="tableName">
          <el-select value-key="id" v-model="form.tableName" allow-create default-first-option filterable
            style="width: 100%" @change="rTbChange" :loading="dbLoading" disabled>
        ...
        <el-form-item v-if="rColumnList && rColumnList.length > 0" label="目标表所有字段：" prop="columnName">
          <div style="width: 100%">
            ...
            <el-checkbox v-model="form.checkAll" :indeterminate="form.isIndeterminate" @change="rHandleCheckAllChange"
              disabled>全选 </el-checkbox>
            <el-checkbox-group v-model="form.columnName" @change="rHandleCheckedChange" style="width: 100%" disabled>
        ...
        <el-form-item v-if="rColumnList && rColumnList.length > 0" label="唯一标识字段：" prop="pkColumnName">
          <el-radio-group v-model="form.pkColumnName" style="width: 100%" disabled>
...
<script>
import { edit, duplicateCheck } from "@/api/dpp/onlDesform/desForm.js";
...
export default {
  ...
  methods: {
    ...
    edit(record) {
      this.form = Object.assign({}, record, { columnName: record.columnName?.split(",") });
      this.visible = true;
      this.getJdbcDs();
