  ...
  <div class="app-container" ref="app-container">
    <div class="pagecont-top" v-show="showSearch">
      <el-form class="btn-style" :model="queryParams" ref="queryRef" :inline="true" label-width="75px">
        <el-form-item label="数据标签类目名称" prop="name" label-width="130">
          <el-input class="el-form-input-width" v-model="queryParams.name" placeholder="请输入数据标签类目名称" clearable
    ...
    <div class="pagecont-bottom">
        ...
        <el-row :gutter="10" class="btn-style">
          <el-col :span="1.5">
            <el-button type="primary" plain icon="Plus" @click="handleAdd"
              v-hasPermi="['att:tagCat:add']">新增</el-button>
          </el-col>
          <el-col :span="1.5">
      ...
      <el-table v-if="refreshTable" height="60vh" v-loading="loading" :data="AttTagCatList" row-key="id"
        ...
        <!--            </template>-->
        <!--          </el-table-column>-->
        <el-table-column label="数据标签类目名称" align="left" prop="name" width="200"
        ...
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width" fixed="right" width="240">
          <template #default="scope">
            <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)"
              v-hasPermi="['att:tagCat:edit']">修改</el-button>
            <el-button link type="primary" icon="Plus" @click="handleAdd(scope.row)"
              v-hasPermi="['att:tagCat:add']">新增</el-button>
            <el-button link type="danger" icon="Delete" @click="handleDelete(scope.row)"
              v-hasPermi="['att:tagCat:remove']">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="类目名称" prop="name">
              <el-input v-model="form.name" placeholder="请输入数据标签类目名称" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
...
<script setup name="TagCat">
import { listAttTagCat, getAttTagCat, delAttTagCat, addAttTagCat, updateAttTagCat } from "@/api/att/cat/tagCat/tagCat";
...
const data = reactive({
  ...
  rules: {
    name: [{ required: true, message: '数据标签类目名称不能为空', trigger: 'blur' }],
...
function getList() {
  loading.value = true;
  listAttTagCat(queryParams.value).then((response) => {
...
function handleStatusChange(row) {
  ...
  proxy.$modal
    ...
    .then(function () {
      updateAttTagCat({ id: row.id, validFlag: row.validFlag }).then((response) => {
...
async function handleUpdate(row) {
  reset();
  // await getTreeselect();
  const response = await listAttTagCat();
  AttTagCatOptions.value = [];
  // 过滤节点的计算属性
  ...
  getAttTagCat(row.id).then((response) => {
    //把createTime过滤掉
    delete response.data.createTime;
    delete response.data.updateTime;
    form.value = response.data;
    open.value = true;
    title.value = '修改数据标签类目';
  });
}
  ...
  proxy.$refs['AttTagCatRef'].validate((valid) => {
    if (valid) {
      if (form.value.id != null) {
        updateAttTagCat(form.value).then((response) => {
          proxy.$modal.msgSuccess('修改成功');
          open.value = false;
          getList();
        });
      } else {
        addAttTagCat(form.value).then((response) => {
