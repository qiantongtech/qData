  ...
  <div class="app-container" ref="app-container">
    <div class="pagecont-top" v-show="showSearch">
      <el-form class="btn-style" :model="queryParams" ref="queryRef" :inline="true">
        <el-form-item :label="td('att.common.documentCatName')" prop="name">
    ...
    <div class="pagecont-bottom">
      ...
      <el-table v-if="refreshTable" height="60vh" v-loading="loading" :data="AttTagCatList" row-key="id"
        ...
        <!--          <el-table-column :label="td('common.texts.number')"  prop="code" :show-overflow-tooltip="{effect: 'light'}" width="160">-->
        <!--            <template #default="scope">-->
        <!--              {{ scope.row.code || '-' }}-->
        <!--            </template>-->
        <!--          </el-table-column>-->
        <el-table-column :label="td('att.documentCat.table.name')" align="left" prop="name" width="400"
        ...
        <el-table-column :label="td('common.texts.remark')" align="left" prop="remark">
          <template #default="scope">
            {{ scope.row.remark || '-' }}
          </template>
        </el-table-column>
...
const data = reactive({
  ...
  rules: {
    name: [{ required: true, message: td('att.documentCat.validations.nameRequired'), trigger: 'blur' }],
    parentId: [{ required: true, message: td('att.documentCat.validations.parentIdRequired'), trigger: 'blur' }],
...
function handleStatusChange(row) {
  ...
  proxy.$modal
    .confirm(td('att.common.confirmStatusChangeGeneric').replace('{status}', text).replace('<name>', row.name).replace('{type}', td('att.common.documentCatName')))
    .then(function () {
...
async function handleUpdate(row) {
  ...
  getAttDocumentCat(row.id).then((response) => {
    ...
    open.value = true;
    title.value = td('att.documentCat.title.edit');
  });
}
