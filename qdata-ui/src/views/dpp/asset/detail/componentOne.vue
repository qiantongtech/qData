<template>
    ...
    <el-table stripe height="60vh" v-loading="loading" :data="daAssetColumnList"
        @selection-change="handleSelectionChange" :default-sort="defaultSort" @sort-change="handleSortChange">
        <el-table-column type="selection" width="55" align="center" />
        <el-table-column v-if="columns[0].visible" label="中文名称" align="left" prop="columnComment" show-overflow-tooltip
            width="185" />
        <el-table-column v-if="columns[1].visible" label="英文名称" align="left" prop="columnName" show-overflow-tooltip
            width="180">
            <template #default="scope">
                {{ scope.row.columnName || '-' }}
            </template>
        </el-table-column>
        <el-table-column v-if="columns[2].visible" label="字段类型" align="left" prop="columnType" show-overflow-tooltip
            width="140">
            <template #default="scope">
                {{ scope.row.columnType || '-' }}
            </template>
        </el-table-column>
        <el-table-column v-if="columns[3].visible" label="字段长度" align="center" prop="columnLength" show-overflow-tooltip
            width="80">
            <template #default="scope">
                {{ scope.row.columnLength || '-' }}
            </template>
        </el-table-column>
        <el-table-column v-if="columns[4].visible" label="小数位" align="center" prop="columnScale" width="80"
            show-overflow-tooltip>
            <template #default="scope">
                {{ scope.row.columnScale || '-' }}
            </template>
        </el-table-column>
        <el-table-column v-if="columns[5].visible" label="是否可空" align="center" prop="nullableFlag" width="80">
            <template #default="scope">
                <el-switch v-model="scope.row.nullableFlag" :active-value="'1'" :inactive-value="'0'" disabled />
            </template>
        </el-table-column>
        <el-table-column v-if="columns[6].visible" label="是否主键" width="100" align="center" prop="pkFlag">
            <template #default="scope">
                <el-switch v-model="scope.row.pkFlag" :active-value="'1'" :inactive-value="'0'" disabled />
            </template>
        </el-table-column>
        <el-table-column v-if="columns[7].visible" label="敏感等级" align="center" prop="sensitiveLevelName" width="100">
            <template #default="scope">
                {{ scope.row.sensitiveLevelName || '无' }}
            </template>
        </el-table-column>
        <el-table-column v-if="columns[8].visible" label="是否代码" width="120" align="left" prop="dataElemCodeFlag"
            show-overflow-tooltip>
            <template #default="scope">
                ...
                <span v-show="scope.row.dataElemCodeFlag">
                    {{ scope.row.dataElemCodeName || '否' }}
                </span>
            </template>
        </el-table-column>

        <el-table-column v-if="columns[9].visible" label="数据元" align="left" prop="relDataElmeFlag" width="120"
            show-overflow-tooltip>
            <template #default="scope">
                <span v-show="scope.row.dataElemCodeFlag">
                    {{ scope.row.relDataElmeName || '否' }}
                </span>
            </template>
        </el-table-column>
        <el-table-column v-if="columns[10].visible" label="字段描述" align="left" prop="description" width="260"
            show-overflow-tooltip>
            <template #default="scope">
    ...
    <el-dialog :title="title" v-model="openDetail" width="800px" :append-to="$refs['app-container']" draggable>
        ...
        <el-form ref="daAssetRef" :model="form" label-width="80px">
            ...
            <el-row :gutter="20">
                <el-col :span="12">
                    <el-form-item label="数据元" prop="relDataElmeFlag">
                        <div>
...
const columns = ref([
    ...
    { key: 8, label: '是否代码', visible: true },
    { key: 9, label: '数据元', visible: true },
    { key: 10, label: '字段描述', visible: true }
]);
