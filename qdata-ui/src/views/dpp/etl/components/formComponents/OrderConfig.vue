    ...
    <el-dialog v-model="visibleDialog" :draggable="true" class="medium-dialog" :title="currentNode?.data?.name"
        ...
        <el-form ref="dpModelRefs" :model="form" label-width="110px" @submit.prevent v-loading="loading">
            ...
            <el-table stripe height="310px" :data="tableFields" v-loading="loadingList" ref="dragTable"
                ...
                <el-table-column label="排序规则" align="center" prop="order">
                    <template #default="scope">
                ...
                <el-table-column label="区分大小写" align="center" prop="caseSensitive" show-overflow-tooltip>
                    <template #default="scope">
                        <el-select v-model="scope.row.caseSensitive" placeholder="请选择">
                ...
                <el-table-column label="locale" align="center" prop="locale">
                    <template #default="scope">
                        <el-select v-model="scope.row.locale" placeholder="请选择">
                            <el-option label="是" :value="true" />
                            <el-option label="否" :value="false" />
                        </el-select>
                    </template>
                ...
                <el-table-column label="排序强度" align="center" prop="collatorStrength">
                    <template #default="scope">
                        <el-select v-model="scope.row.collatorStrength" placeholder="请选择">
                ...
                <el-table-column label="预排序" align="center" prop="presorted">
                    <template #default="scope">
                        <el-select v-model="scope.row.presorted" placeholder="请选择">
                            <el-option label="是" :value="true" />
                            <el-option label="否" :value="false" />
                        </el-select>
                    </template>
...
const saveData = async () => {
    try {
        ...
        tableFields.value = tableFields.value.map(item => ({
            ...
            locale: typeof item.locale === 'boolean' ? item.locale : true,
            collatorStrength: item.collatorStrength ?? 0,
            presorted: typeof item.presorted === 'boolean' ? item.presorted : false,
        }));
