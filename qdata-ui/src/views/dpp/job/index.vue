<template>
  <div class="app-container" ref="app-container">
    ...
    <GuideTip tip-id="dpp/tasks.list" />
        ...
        <div class="pagecont-top" v-show="showSearch">
          <el-form class="btn-style" :model="queryParams" ref="queryRef" :inline="true" label-width="75px"
            ...
            <el-form-item label="作业名称" prop="name">
              <el-input class="el-form-input-width" v-model="queryParams.name" placeholder="请输入作业名称" clearable
                @keyup.enter="handleQuery" />
            </el-form-item>
            <el-form-item label="作业状态" prop="status">
              <el-select v-model="queryParams.status" placeholder="请选择作业状态" clearable class="el-form-input-width">
        ...
        <div class="pagecont-bottom">
          <div class="justify-between mb15">
            <el-row :gutter="15" class="btn-style">
              <el-col :span="1.5">
                <el-button type="primary" plain @click="routeTo('/dpp/job/add', '1')"
                  >
          ...
          <el-table stripe v-loading="loading" :data="dppEtlTaskList" :default-sort="defaultSort"
            ...
            <el-table-column v-if="getColumnVisibility(1)" label="作业名称" :show-overflow-tooltip="{ effect: 'light' }"
              align="left" prop="name" width="200">
              <template #default="scope">
                {{ scope.row.name || "-" }}
              </template>
            </el-table-column>
            <el-table-column v-if="getColumnVisibility(7)" label="作业类目" :show-overflow-tooltip="true" align="left"
              prop="catName" width="125">
              <template #default="scope">
                {{ scope.row.catName || "-" }}
              </template>
            </el-table-column>
            <el-table-column v-if="getColumnVisibility(2)" label="描述" :show-overflow-tooltip="{ effect: 'light' }"
              align="left" prop="description" width="200">
              <template #default="scope">
                {{ scope.row.description || "-" }}
              </template>
            </el-table-column>
            <el-table-column v-if="getColumnVisibility(3)" label="作业状态" width="80" align="left" prop="releaseState">
              <template #header>
                <div class="justify-center">
                  <span style="margin-right: 2px;">作业状态</span>
            ...
            <el-table-column v-if="getColumnVisibility(4)" label="调度状态" width="80" align="left" prop="schedulerState">
              <template #header>
                <div class="justify-center">
                  <span style="margin-right: 2px;">调度状态</span>
                  <el-tooltip effect="light" content="作业状态为启用后可开启调度" placement="top">
                    <el-icon class="tip-icon">
                      <InfoFilled />
                    </el-icon>
                  </el-tooltip>
            ...
            <el-table-column v-if="getColumnVisibility(5)" label="执行策略" width="80"
            ...
            <el-table-column v-if="getColumnVisibility(6)" label="调度周期" :show-overflow-tooltip="{ effect: 'light' }"
            ...
            <el-table-column v-if="getColumnVisibility(8)" label="最近运行时间" width="160" align="left"
            ...
            <!-- <el-table-column v-if="getColumnVisibility(4)" width="100" label="最近执行结果"  align="left" prop="status">
              <template #default="scope">
            ...
            <el-table-column label="操作" align="center" class-name="small-padding fixed-width" fixed="right" width="250">
              <template #default="scope">
                <el-button link type="primary" icon="Edit" :disabled="scope.row.status == 1"
                  @click="routeTo('/dpp/job/edit', scope.row)" >配置作业</el-button>
                <el-button link type="primary" icon="view" @click="
                  routeTo('/dpp/job/detail', {
                    ...scope.row,
                    info: true,
                  })
                  " >详情</el-button>
    ...
    <instance :visible="DataView" @update:visible="DataView = $event" @confirm="submitForm" :data="form" title="运行实例" />
...
function handleschedulerState(id, row, e) {
  ...
  // 弹出确认框
  proxy.$modal
    .confirm('确认要"' + text + '","' + row.name + '"作业调度状态吗？')
    .then(function () {
      loading.value = true;
      // 调用后台接口更新调度状态
      updateReleaseSchedule({
...
function handleStatusChange(id, row, e) {
  ...
  // 弹出确认框
  proxy.$modal
    .confirm('确认要"' + text + '","' + row.name + '"作业任务吗？')
    .then(function () {
...
const columns = ref([
  ...
  { key: 3, label: "作业状态", visible: true },
  { key: 4, label: "调度状态", visible: true },
  { key: 5, label: "执行策略", visible: true },
  { key: 6, label: "调度周期", visible: true },
  { key: 7, label: "作业类目", visible: true },
  { key: 8, label: "最近运行时间", visible: true },
