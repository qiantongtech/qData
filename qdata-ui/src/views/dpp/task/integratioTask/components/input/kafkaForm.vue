      ...
      <el-row :gutter="20">
        <el-col :span="12">
          ...
          >
            ...
            <div v-else class="form-readonly">
              {{ form.taskParams.readerDatasource?.datasourceId || "-" }}
            </div>
          </el-form-item>
        </el-col>
        <el-col :span="12">
        ...
        >
          <template #default="scope">
            <el-button
              ...
              type="primary"
              icon="Edit"
              @click="openDialog({ ...scope.row, index: scope.$index + 1 })"
              >修改</el-button
            >
            <el-button
              type="danger"
...
const saveData = async () => {
  try {
    ...
    (taskParams.tableFields = ColumnByAssettab.value.map((item) => {
      return {
        ...item,
...
watchEffect(() => {
  if (props.visible) {
    ...
    ColumnByAssettab.value =
      deepCopy(props.currentNode.data.taskParams.tableFields) || [];
    if (props.currentNode.data.code) {
      handleDatasource(
        props.currentNode.data.taskParams?.readerDatasource || ""
      );
    }
  } else {
    off();
