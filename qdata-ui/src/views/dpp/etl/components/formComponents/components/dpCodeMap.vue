<template>
    ...
    <el-table stripe height="300px" v-loading="loading" :data="dpCodeMapList">
        <el-table-column label="原始值" :show-overflow-tooltip="true" align="left" prop="originalValue">
            <template #default="scope">
                {{ scope.row.originalValue || '-' }}
            </template>
        </el-table-column>
        <el-table-column label="字典名" :show-overflow-tooltip="true" align="left" prop="dictName">
            <template #default="scope">
                {{ scope.row.dictName || '-' }}
            </template>
        </el-table-column>
        <el-table-column label="字典值" :show-overflow-tooltip="true" align="center" prop="dictValue">
            <template #default="scope">
                {{ scope.row.dictValue || '-' }}
            </template>
        </el-table-column>
