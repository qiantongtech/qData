      ...
      <el-main>
        ...
        <div class="pagecont-bottom">
          ...
          <el-table stripe height="60vh" v-loading="loading" :data="DppQualityTaskEvaluateList">
            ...
            <el-table-column v-if="getColumnVisibility(5)" align="center" prop="status" width="150">
              <template #header>
            ...
            <el-table-column v-if="getColumnVisibility(4)" label="上次执行时间" align="center" prop="lastExecuteTime"
              ...
              <template #default="scope">
                {{ parseTime(scope.row.lastExecuteTime, '{y}-{m}-{d} {h}:{i}:{s}') || '-' }}
              </template>
            </el-table-column>
            <el-table-column label="操作" align="center" class-name="small-padding fixed-width" fixed="right" width="220">
              <template #default="scope">
                ...
                <el-button link type="primary" icon="Edit" @click="routeTo('/da/dataQuality/dataQualityTasks/components/qualityTaskEdit', {
                  ...scope.row,
                })" v-hasPermi="['da:dataQualityTasks:edit']" :disabled="scope.row.status != 1">
                  配置</el-button>
                <el-button link type="primary" icon="view" @click="
                  ...
                  " v-hasPermi="['da:dataQualityTasks:info']">详情</el-button>
                ...
                <el-popover placement="bottom" :width="150" trigger="click">
                  ...
                  <div style="width: 100px" class="butgdlist">
                    <el-button link type="primary" icon="VideoPlay" style="padding-left: 14px"
                      @click="handleExecuteOnce(scope.row)" v-hasPermi="['da:qa:dataQualityTasks:once']"
                      :disabled="scope.row.status == 1">执行一次</el-button>
                    <el-button link type="primary" icon="Stopwatch" @click="handleDataView(scope.row)"
                      v-hasPermi="['da:dataQualityTasks:edit']">执行记录</el-button>
                    <el-button link type="danger" icon="Delete" :disabled="scope.row.status != 1"
                      @click="handleDelete(scope.row)" v-hasPermi="['da:dataQualityTasks:remove']">删除</el-button>
                    <el-button link icon="Operation" @click="handleJobLog(scope.row)" type="primary"
                      :disabled="scope.row.status != 1" v-hasPermi="['da:dataQualityTasks:schedule']">调度周期</el-button>
                  </div>
                </el-popover></template>
            </el-table-column>
            <template #empty>
