<template>
  <el-dialog v-model="visibleDialog" draggable class="dialog" :title="title" style="width: 1500px" destroy-on-close>
    <el-table stripe height="380px" v-loading="loading" :data="jobLogList">
      ...
      <el-table-column label="质量评分" align="center" prop="score">
        <template #default="scope">
          {{ scope.row.score || '-' }}
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width" fixed="right" width="240">
        <template #default="scope">
          ...
          <el-button link type="primary" icon="view" @click="
            ...
            " v-hasPermi="['dpp:dppQualityLog:dppqualitylog:edit']">详情</el-button>
        </template>
      </el-table-column>
...
import { listDaDiscoveryTaskLog, logDetailCat } from '@/api/da/discovery/daDiscoveryTaskLog';
...
import { listDppQualityLog, getDppQualityLog, delDppQualityLog, addDppQualityLog, updateDppQualityLog } from "@/api/da/dppQuality/DppQualityLog";
