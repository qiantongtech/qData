  ...
  <el-dialog v-model="visibleDialog" :draggable="true" class="medium-dialog" :title="currentNode?.data?.name"
    ...
    <el-form ref="dpModelRefs" :model="form" label-width="110px" @submit.prevent v-loading="loading" :disabled="info">
      ...
      <el-table stripe height="310px" :data="tableFields" v-loading="loadingList" ref="dragTable" row-key="columnName">
        ...
        <el-table-column label="替换字段名称" align="left" prop="sourceField" :show-overflow-tooltip="{ effect: 'light' }">
          <template #default="scope">
            <el-select v-model="scope.row.sourceField" placeholder="请选择字段" style="flex: 1">
              <el-option v-for="item in inputFields" :key="item.value" :label="item.label" :value="item.columnName"
                :disabled="scope.row.columnName === item.columnName" />
            </el-select>
          </template>
        </el-table-column>
...
function handleAddField() {
  ...
  const lastEmpty = tableFields.value.find((item) => !item.sourceField);
  if (lastEmpty) {
    proxy.$message.warning("请先填写当前替换字段名称");
    return;
  }
...
const saveData = async () => {
  try {
    ...
    taskParams.tableFields = tableFields.value.map((field) => ({ ...field, targetField: field.columnName }));
