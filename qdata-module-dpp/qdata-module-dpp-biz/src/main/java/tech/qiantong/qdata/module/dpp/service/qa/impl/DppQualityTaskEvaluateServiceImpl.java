                    ...
                    if (isUpdateSupport) {
                        if (dppQualityTaskEvaluateId != null) {
                            ...
                            if (existingDppQualityTaskEvaluate != null) {
                                ...
                                successMessages.add(MessageUtils.messageWithFallback("dpp.import.update.success",
                                        "Data update successful, ID {0} {1} record.", dppQualityTaskEvaluateId, MessageUtils.messageWithFallback("dpp.entity.quality.task.evaluation.rule", "Data quality task evaluation rule")));
                            } else {
                                failureNum++;
                                failureMessages.add(MessageUtils.messageWithFallback("dpp.import.update.fail",
                                        "Data update failed, ID {0} {1} record does not exist.", dppQualityTaskEvaluateId, MessageUtils.messageWithFallback("dpp.entity.quality.task.evaluation.rule", "Data quality task evaluation rule")));
                            }
                        } else {
                            failureNum++;
                        ...
                        if (existingDppQualityTaskEvaluate == null) {
                            ...
                            successMessages.add(MessageUtils.messageWithFallback("dpp.import.insert.success",
                                    "Data insert successful, ID {0} {1} record.", dppQualityTaskEvaluateId, MessageUtils.messageWithFallback("dpp.entity.quality.task.evaluation.rule", "Data quality task evaluation rule")));
                        } else {
                            failureNum++;
                            failureMessages.add(MessageUtils.messageWithFallback("dpp.import.insert.fail",
                                    "Data insert failed, ID {0} {1} record already exists.", dppQualityTaskEvaluateId, MessageUtils.messageWithFallback("dpp.entity.quality.task.evaluation.rule", "Data quality task evaluation rule")));
                        }
                    }
                } catch (Exception e) {
                    failureNum++;
