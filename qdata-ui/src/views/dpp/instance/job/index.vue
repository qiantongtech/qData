    ...
    <el-container style="90%">
      ...
      <el-main>
        <div class="pagecont-top" v-show="showSearch">
          <el-form class="btn-style" :model="queryParams" ref="queryRef" :inline="true" label-width="120px"
            v-show="showSearch" @submit.prevent>
            <el-form-item label="作业实例名称" prop="name">
              <el-input class="el-form-input-width" v-model="queryParams.name" placeholder="请输入作业实例名称" clearable
        ...
        <div class="pagecont-bottom">
          ...
          <el-table stripe v-loading="loading" :data="dppEtlTaskList" row-key="dataId" lazy :load="load"
            :default-expand-all="isExpandAll" :tree-props="{ children: 'children', hasChildren: 'hasChildren' }">
            <el-table-column label="作业实例名称" :show-overflow-tooltip="{ effect: 'light' }" align="left" prop="name"
              width="400">
              <template #default="scope">
                {{ scope.row.name || "-" }}
              </template>
            </el-table-column>
            <el-table-column label="任务名称" :show-overflow-tooltip="{ effect: 'light' }" align="left" prop="taskName"
              width="300">
              <template #default="scope">
                {{ scope.row.taskName || "-" }}
              </template>
            </el-table-column>
            <el-table-column label="执行类型" width="100" :show-overflow-tooltip="{ effect: 'light' }" align="left"
              prop="commandType">
              <template #default="scope">
            ...
            <el-table-column label="执行状态" width="80" :show-overflow-tooltip="{ effect: 'light' }" align="center"
              fixed="right" prop="status">
              <template #default="scope">
            ...
            <!-- <el-table-column label="调度时间" :show-overflow-tooltip="{effect: 'light'}" align="center" prop="scheduleTime" width="160">
              <template #default="scope">
                {{ scope.row.scheduleTime || "-" }}
              </template>
            </el-table-column> -->
            <el-table-column label="开始时间" :show-overflow-tooltip="{ effect: 'light' }" align="center" prop="startTime"
              width="160">
              <template #default="scope">
                <span>{{
            ...
            <el-table-column label="结束时间" :show-overflow-tooltip="{ effect: 'light' }" align="center" prop="endTime"
              width="160">
              <template #default="scope">
                <span>{{
            ...
            <el-table-column label="运行时长" :show-overflow-tooltip="{ effect: 'light' }" align="left" prop="duration"
              width="80">
              <template #default="scope">
                {{ scope.row.duration || "-" }}
              </template>
            ...
            <el-table-column label="运行次数" :show-overflow-tooltip="{ effect: 'light' }" align="center" prop="runTimes"
              width="80">
              <template #default="scope">
                {{ scope.row.runTimes || "-" }}
              </template>
            ...
            <el-table-column label="操作" header-align="center" class-name="small-padding fixed-width" fixed="right"
              ...
              <template #default="scope">
                <el-button v-if="scope.row.dataType == '1'" :disabled="scope.row.status !== '7' &&
                  ...
                  scope.row.status !== '6' &&
                  scope.row.status !== '5'
                  " link type="primary" @click="repeatTask(scope.row.id, scope.row.name)">
                  <img src="@/assets/icons/svg/reload.svg" alt="" style="margin-right: 6px" />
                  重跑
                </el-button>
                ...
                <el-button v-if="scope.row.dataType != '1' && scope.row.dataType != '5'" icon="Download" link
                  type="warning" @click="downloadLog(scope.row)" @mousedown="(e) => e.preventDefault()">
                  下载日志
                </el-button>
              </template>
            </el-table-column>
...
function getDeptTree() {
  listAttJobCat({ type: 4, validFlag: true }).then((response) => {
...
function repeatTask(taskInstanceId, taskName) {
  ElMessageBox.confirm(`确定要重跑【${taskName}】吗？`, "操作确认", {
    confirmButtonText: "确定",
    cancelButtonText: "取消",
    type: "warning",
  }).then(() => {
    // 用户点击确认
    execute(taskInstanceId, "REPEAT_RUNNING").then((response) => {
      if (response.code === 200) {
        getList();
      }
    });
