...
| 场景 | 描述 | 典型客户类型 |
| --- | --- | --- |
| **ETL 数据集成** | 通过可视化方式配置数据接入、清洗转换和输出流程，任务可按场景选择 DataX 或 Spark 执行引擎，支撑轻量同步、关联数据获取、数据导入和大规模离线处理。 | 数据研发团队、软件公司 |
| **数据开发调度** | 支持 SQL 数据处理任务开发，新增任务时可选择 Quartz 或 DolphinScheduler，兼顾轻量周期调度与可视化任务编排。 | 数据研发团队、平台运维团队 |
| **数据治理建设** | 统一管理数据标准、数据模型、元数据、数据质量和数据资产，建立基础治理体系。 | 政府机构、集团企业、科研院校 |
| **元数据采集管理** | 支持采集任务按实际部署选择 Quartz 或 DolphinScheduler 调度，便于灵活配置采集执行策略。 | 数据治理团队、数据管理部门 |
...
| **可视化 ETL** | 支持通过可视化方式配置数据接入、清洗转换和输出流程，并可在 DataX 与 Spark 之间选择执行引擎，降低不同规模数据集成任务的开发门槛。 |
| **执行引擎灵活** | DataX 适合单机轻量数据同步、关联数据获取与导入场景，Spark 适合大规模离线处理与复杂计算任务。 |
| **调度方式可选** | 数据开发任务和采集任务可选择 Quartz 或 DolphinScheduler，既支持轻量级本地调度，也支持集中式任务编排、依赖管理和调度治理。 |
